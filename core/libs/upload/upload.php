...
abstract class Upload {
    ...
    public function isUploaded() {
        ...
        if ($_FILES[$this->_name]['error'] > 0) {
            $error = array(UPLOAD_ERR_INI_SIZE => 'il file supera la dimensione massima ('
                . ini_get('upload_max_filesize') .
                'b) consentita dal server',
                UPLOAD_ERR_FORM_SIZE => 'il file supera la dimensione massima',
                UPLOAD_ERR_PARTIAL => 'E \'stato parzialmente caricato il file',
                UPLOAD_ERR_NO_FILE => 'Essa non ha caricato alcun file',
                UPLOAD_ERR_NO_TMP_DIR => 'Non si trova la directory dei file temporanei',
                UPLOAD_ERR_CANT_WRITE => 'Non è riuscito a scrivere il file su disco',
                UPLOAD_ERR_EXTENSION => 'una estensione PHP fermato il caricamento di file');

            Flash::error('Errore: ' . $error[$_FILES[$this->_name]['error']]);
            return FALSE;
        }
        return TRUE;
    }
    ...
    protected function _overwrite($name) {
        ...
        if (file_exists("$this->_path/$name")) {
            Flash::error('Errore: Questo file esiste già. E non ti è permesso di scrivere');
            return FALSE;
        }
        return TRUE;
    }
